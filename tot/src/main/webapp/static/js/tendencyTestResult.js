$(document).ready(function () {
    ...
    const memId = $('.result-container input[data-memId]').data('memid') || 'guest';  // memId가 없으면 'guest'로 기본값 설정
    console.log(memId);
...
function updateResult(resultType) {
    const resultData = {
        ...
        '실속주의자': {
            image: `static/image/testResult_practical.png`,
            description: '예산안에서 완벽하게 즐기고 싶은 당신은 실속주의자입니다.\n자신이 정한 예산안에서 하고 싶은 일정을 다 소화 했을 때\n 여행만족도가 MAX에 달하는 편'
        },
        '자유로운영혼': {
            image: `static/image/testResult_freeSpirit.png`,
...
function saveResult(resultType) {
    ...
    if (memId === 'guest') {
        ...
        fetch('/tot/saveTendencyTestResult', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
function getRecommendation() {
    const memId = sessionStorage.getItem('memId'); // memId 가져오기

    if (memId === 'guest') {
        const userConfirmed = confirm('로그인 후 성향에 맞는 여행지 추천을 받을 수 있습니다. 로그인 페이지로 이동하시겠습니까?');
